//
// Advent of Code 2018, day 24, part two
//

// At certain boost levels you run into stalemates, so we check if there is
// any progress in the attack phase.
...
bool attack_target(Group &grp, Group &target)
{
	int damage = effective_power(grp);
...
bool attack_targets(std::list<Group> &attackers, std::list<Group> &defenders)
{
		...
		if (attack_grp != attackers.end() && defend_grp != defenders.end()) {
			if (attacks_before(*attack_grp, *defend_grp)) {
				if (attack_grp->targeting != defenders.end()) {
					if (attack_target(*attack_grp, *attack_grp->targeting)) {
						progress = true;
					}
				}
				++attack_grp;
				...
				if (defend_grp->targeting != attackers.end()) {
					if (attack_target(*defend_grp, *defend_grp->targeting)) {
						progress = true;
					}
				}
				++defend_grp;
...
std::pair<bool, int> simulate_combat(std::list<Group> immune_system, std::list<Group> infection, int boost)
{
	for (auto &grp : immune_system) {
		grp.ap += boost;
	}
	...
	while (has_groups_alive(immune_system) && has_groups_alive(infection)) {
		...
		if (!attack_targets(immune_system, infection)) {
			return {false, -1};
		}
	...
	int immune_system_units = units_alive(immune_system);
	int infection_units = units_alive(infection);

	return {immune_system_units > 0, immune_system_units + infection_units};
}
...
int main()
{
	...
	for (int boost = 1; boost < 100; ++boost) {
		auto [immune_system_won, num_alive] = simulate_combat(immune_system, infection, boost);

		if (immune_system_won) {
			std::cout << "immune system won with boost " << boost << " and " << num_alive << " units left\n";
			break;
		}
	}
